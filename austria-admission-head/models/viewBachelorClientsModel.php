			...
			while ($row = mysqli_fetch_assoc($clientData_ex)) {
				...
				<tr id="<?php echo $row['client_id']; ?>" class="<?php echo $style;?>">
					...
					<td class="breakTD">
						...
						<br>
						<?php
							$query="SELECT COUNT(aus_direct_applied_status) AS totalAssign, SUM(CASE WHEN aus_direct_applied_status='9' THEN 1 ELSE 0 END) AS assignedNo FROM austria_clients_programs".$_SESSION['dbNo']." WHERE close='1' AND status='1' AND aus_change_program_status='0' AND aus_clients_id='".$clientID."' AND aus_university_name='".$uniName."' ";
							$result = mysqli_query($con, $query);
							$data = mysqli_fetch_array($result);
							$totalNo = $data['totalAssign'];
							$assignedNo = $data['assignedNo'];
