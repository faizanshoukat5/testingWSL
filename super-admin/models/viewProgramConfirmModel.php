...
if (isset($_POST['checkcountryName'])) {
	...
	$whereCondition = "close='1' AND status='1' AND admin_status='1' AND case_status='0' AND change_status='0' AND client_pro_confirm_status='1' ";
		...
		<table id="datatable" class="table table-bordered nowrap text-center" style="width: 100%; table-layout: fixed;">
			...
			while ($row = mysqli_fetch_assoc($clientData_ex)) {
				...
				<tr id="<?php echo $row['client_id']; ?>">
					...
					<td>
					<!-- confirmation of program form client -->
					<button type="button" class="btn <?= $row['client_pro_confirm_status']==1 ? 'btn-success' : 'btn-outline-dark'?> btn-sm" data-toggle="tooltip" data-placement="top" title="Confirm Client Programs" onclick="programClients(<?= $row['client_id'] ?>);"> <i class="mdi <?= $row['client_pro_confirm_status']==1 ? 'mdi-check-circle' : 'mdi-close-circle' ?>"></i> Prog </button>
					<br>

					<?php if($row['client_pro_confirm_status']==1 && $row['client_pay_confirm_status'] ==1 && $row['client_pro_confirm_status']==1 && $row['client_document_status']==1) {?>
						<span class="badge badge-success" data-toggle="tooltip" data-placement="top" title="Forwarded to Admission Head">Forwarded</span>
					<?php }elseif($row['client_pro_confirm_status']==1 || $row['client_pay_confirm_status']==1 || $row['client_pro_confirm_status']==1 || $row['client_document_status']==1){?>
						<span class="badge badge-dark" data-toggle="tooltip" data-placement="top" title="Process">Process</span>
					<?php }if($row['client_pro_confirm_status']==0 && $row['client_pay_confirm_status']==0 && $row['client_pro_confirm_status']==0 && $row['client_document_status']==0) {?>
						<span class="badge badge-info" id="blink" data-toggle="tooltip" data-placement="top" title="New Client">New</span>
					<?php }?>
					</td>
					<td>
						<button type="button" class="btn btn-outline-success btn-sm" data-toggle="tooltip" data-placement="top" title="Track this Client" onclick="ViewClients(<?php echo $row['client_id'];?>);"><i class="mdi mdi-eye"></i></button> 
					</td>
				</tr>
				<?php
				$sr--;
