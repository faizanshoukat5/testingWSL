		...
		<table id="datatable" class="table table-bordered nowrap text-center" style="width: 100%; table-layout: fixed;">
			...
			while ($row = mysqli_fetch_assoc($clientData_ex) ) {
				...
				<tr id="<?php echo $row['client_id']; ?>">
					...
					<td>
					<?php
					$query = "SELECT COUNT(italy_sops_status) AS totalAssign, SUM(CASE WHEN (italy_sops_status='1' || italy_sops_status='2' || italy_sops_status='3' || italy_sops_status='4') THEN 1 ELSE 0 END) AS assignedNo FROM italy_clients_programs".$_SESSION['dbNo']." WHERE close='1' AND italy_clients_id='".$clientID."' AND italy_sops_assign_to='".$wtID."' ";
