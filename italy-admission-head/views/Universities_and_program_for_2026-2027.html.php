	...
	<div class="card-body">
		...
		<div id="showPrograms" style="display: none;">
			...
			<div class="row">
				...
				<input type="hidden" name="" value="2026" id="proIntake">
				...
				<div class="form-group col-md-6 col-lg-4">
					...
					<select class="form-control" data-toggle="select2" name="university-name" required="required" autocomplete="off" onchange="viewClientsFilterPro(1)" id="university-name">
						...
						<?php
						$uniDetails = "SELECT * FROM italy_add_universities".$_SESSION['dbNo']." WHERE status='1' AND close='1' AND italy_uni_intake='2026' GROUP BY italy_uni_name ORDER BY italy_add_id ASC";
				...
				<div class="form-group col-md-6 col-lg-2">
					...
					<select class="form-control" name="client-degree-pro" required="required" autocomplete="off" onchange="viewClientsFilterPro(1)" id="client-degree-pro"> 
						...
						<option value="mbbs" <?= isset($_GET['client-degree-pro']) && $_GET['client-degree-pro'] == 'mbbs' ? 'selected' : '' ?>>MBBS</option>
						<option value="phd" <?= isset($_GET['client-degree-pro']) && $_GET['client-degree-pro'] == 'phd' ? 'selected' : '' ?>>PHD</option>
					</select>
				</div>
				<div class="form-group col-md-6 col-lg-3">
					<label>Application Process</label>
					<select class="form-control" name="application-process-pro" required="required" autocomplete="off" onchange="viewClientsFilterPro(1)" id="application-process-pro"> 
						<option value="all" <?= isset($_GET['application-process-pro']) && $_GET['application-process-pro'] == 'all' ? 'selected' : '' ?>>All</option>
						<option value="Dream Apply" <?= isset($_GET['application-process-pro']) && $_GET['application-process-pro'] == 'Dream Apply' ? 'selected' : '' ?>>Dream Apply</option>
						<option value="Direct Apply" <?= isset($_GET['application-process-pro']) && $_GET['application-process-pro'] == 'Direct Apply' ? 'selected' : '' ?>>Direct Apply</option>
						<option value="Direct Pre Enrollment" <?= isset($_GET['application-process-pro']) && $_GET['application-process-pro'] == 'Direct Pre Enrollment' ? 'selected' : '' ?>>Direct Pre Enrollment</option>
					</select>
				</div>
				<div class="form-group col-md-6 col-lg-3">
				...
				<div class="form-group col-md-6 col-lg-3">
					...
					<select class="form-control" data-toggle="select2" name="cgpa-per" required="required" autocomplete="off" onchange="viewClientsFilterPro(1)" id="cgpa-per">
						...
						<?php
						$cgpaDetails = "SELECT italy_ad_cgpa FROM italy_add_programs_details".$_SESSION['dbNo']." WHERE status='1' AND close='1' AND italy_ad_intake='2026' GROUP BY italy_ad_cgpa ORDER BY FIELD(italy_ad_cgpa, '2.0') DESC, italy_ad_cgpa ASC";
		...
		<div id="showUni" style="display: none;">
			...
			<div class="row">
				...
				<input type="hidden" name="" value="2026" id="uniIntake-uni">
				<div class="form-group col-md-6 col-lg-3">
					<label>Application Process</label>
...
<script type="text/javascript">
	...
	function viewClientsFilterPro(pagePro, call) {
		...
		var applicationProcess = $("#application-process-pro").val();
		...
		if (clientDegree!=='all') {
			params.set('client-degree-pro', clientDegree);
		}else{
			params.delete('client-degree-pro');
		}
		if (applicationProcess!=='all') {
			params.set('application-process-pro', uniName);
		}else{
			params.delete('application-process-pro');
		}
		if (activeUni!=='all') {
