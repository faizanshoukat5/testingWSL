...
<div class="card">
	<div class="card-body">
		...
		<div class="row mt-2">
			<?php
			$sr=1;
			$AddFaq = "SELECT * from austria_faqs_questions WHERE close='1' AND status='1' ";
					...
					<div class="col-md-6">
						<div id="accordion" class="mb-3">
							<div class="card mb-0 accordion-item">
								<div class="card-header" id="heading<?php echo $row['aus_faq_id'];?>">
									<h5 class="m-0">
										<a href="#collapse<?php echo $row['aus_faq_id'];?>" class="text-dark" data-toggle="collapse" aria-expanded="<?php echo $isFirst ? 'true' : 'false';?>" aria-controls="collapse<?php echo $row['aus_faq_id'];?>">
											<?php echo $sr;?>) <?php echo $row['aus_faq_title']; ?>
										</a>
									</h5>
								</div>

								<div id="collapse<?php echo $row['aus_faq_id'];?>" class="collapse<?php echo $isFirst ? ' show' : '';?>"  aria-labelledby="heading<?php echo $row['aus_faq_id']; ?>"  data-parent="#accordion">
									<div class="card-body">
										<?php echo $row['aus_faq_answer']; ?>
										<div class="float-right">
											<button type="button" class="btn btn-outline-primary btn-sm" data-toggle="tooltip" data-placement="top" title="Edit" onclick="editFAQs(<?php echo $row['aus_faq_id']; ?>);"><i class="mdi mdi-square-edit-outline"></i></button>
										</div>
									</div>
								</div>
							</div>
