...
if (isset($_POST['checkconvertStatus'])) {
	...
	if ($checkApplication!="all" ) {
		if($checkApplication=='Assign But Not Applied'){
			$whereCondition .= " AND icp.italy_proof_screenshot1='' AND icp.italy_assign_status='1' ";
		}
		if ($checkApplication=='Waiting for Admission decision') {

			$whereCondition .= " AND icp.italy_info_client_status='6' AND icp.italy_applied_status='7' AND icp.italy_assign_status='1'";
		}
		if($checkApplication=='Acceptance Letter Received Clients'){
			$whereCondition .= " AND (icp.italy_dream_program1_status='Acceptance' || icp.italy_dream_program2_status='Acceptance')  AND icp.italy_assign_status='1' ";
		}
		if($checkApplication=='University Admission Rejected Clients'){
			$whereCondition .= " AND (icp.italy_dream_program1_status='Rejection' || icp.italy_dream_program2_status='Rejection') AND icp.italy_assign_status='1'";
		}
	}
	?>
