...
if (isset($_POST['checkcountryName'])) {
	...
	$whereCondition = "close='1' AND status='1' AND admin_status='1' AND case_status='0' AND change_status='0' AND client_pay_confirm_status='0' ";
		...
		<table id="datatable" class="table table-bordered nowrap text-center" style="width: 100%; table-layout: fixed;">
			<thead>
				<tr>
					...
					<th style="width: 150px;">Accounts</th>
			...
			while ($row = mysqli_fetch_assoc($clientData_ex)) {
				...
				<tr id="<?php echo $row['client_id'];?>">
					...
					<td>
					<!-- Bank Payment Confirmation Button -->
					<button type="button" class="btn <?= $row['bank_pay_status']==1 ? 'btn-success' : 'btn-outline-warning' ?> btn-sm" data-toggle="tooltip" data-placement="top" title="Payment Confirm form Bank" onclick="bankConfirm(<?= $row['client_id'] ?>)"> <i class="mdi <?= $row['bank_pay_status']==1 ? 'mdi-check-circle' : 'mdi-close-circle' ?>"></i> P.C.B </button>
					<!-- Manager Receipt Confirmation Button -->
					<button type="button" class="btn <?= $row['manager_pay_status']==1 ? 'btn-success' : 'btn-outline-info' ?> btn-sm" data-toggle="tooltip" data-placement="top" title="Account Manager Receipt" onclick="managerConfirm(<?= $row['client_id'] ?>)"> <i class="mdi <?= $row['manager_pay_status']==1 ? 'mdi-check-circle' : 'mdi-close-circle' ?>"></i> M.R </button>
					<br>
					<!-- payment confirm -->
					<button type="button" class="btn <?= $row['client_pay_confirm_status']==1 ? 'btn-success' : 'btn-outline-secondary' ?> btn-sm mt-1" data-toggle="tooltip" data-placement="top" title="Payment Confirmation Send to Client" onclick="payConfirmClient(<?php echo $row['client_id'];?>)"><i class="mdi <?= $row['client_pay_confirm_status']==1 ? 'mdi-check-circle' : 'mdi-close-circle' ?>"></i> P.C</button>
					<br>
					<?php if($row['client_pro_confirm_status'] == 1 && $row['client_pay_confirm_status'] ==1 && $row['ack_confirm_status'] == 1 && $row['client_document_status'] ==1) {?>
						<span class="badge badge-success" data-toggle="tooltip" data-placement="top" title="Forwarded to Admission Head">Forwarded</span>
					<?php }elseif($row['client_pro_confirm_status']==1 || $row['client_pay_confirm_status']==1 || $row['ack_confirm_status']==1 || $row['client_document_status']==1){?>
						<span class="badge badge-dark" data-toggle="tooltip" data-placement="top" title="Process">Process</span>
					<?php }if($row['client_pro_confirm_status']==0 && $row['client_pay_confirm_status']==0 && $row['ack_confirm_status']==0 && $row['client_document_status']==0) {?>
