<script type="text/javascript">
	...
	// Save All Clients Data using AJAX
	...
	// Assign Client to Admission Whatsapp Agent
	function assignAgentCleints(id) {
		var id = id;
		$.ajax({
			type: "POST",
			url: "getState.php",
			data:'assignWhatsAppAgent='+id,
			success: function(data){
				$(".showModalTitle").html('Assign To Admission WhatsApp Agent');
				$(".showModalClient").html(data);
				$("#showModalClient").modal('show');
			}
		});
	};
	// Whatsapp Agent Assign Password Check
	function agentPasswordCheck() {
		var enterPassword = $("#enterPassword").val();
		if(enterPassword=='000'){
			$("#agentBtn").prop('disabled', false);
		}
		else{
			$("#agentBtn").prop('disabled', true);
		}
	}
	...
	function delAgent(id) {
		...
		$.ajax({
			type: "POST",
			url: "getState.php",
			data:'deleteWhatsAppAgent='+id,
			success: function(data){
				$(".showModalTitle").html('Delete Admission WhatsApp Agent');
				$(".showModalClient").html(data);
				$("#showModalClient").modal('show');
			}
		});
	...
	function dueAfterAcceptance(id) {
		...
		$.ajax({
			...
			success: function(data){
				$(".showModalTitle1").html('Due After Acceptance');
	...
	function selfAcceptance(id) {
		...
		$.ajax({
			...
			success: function(data){
				$(".showModalTitle1").html('Self Received Acceptance');
				$(".showModalClient1").html(data);
				$("#showModalClient1").modal('show');
				$('#showModalClient1').off('shown.bs.modal').on('shown.bs.modal', function () {
					document.querySelectorAll('.agreement-container').forEach(function(agreementContainer) {
