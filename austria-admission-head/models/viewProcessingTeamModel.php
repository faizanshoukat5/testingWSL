			...
			while ($row = mysqli_fetch_assoc($clientData_ex)) {
				...
				<tr id="<?php echo $row['client_id']; ?>">
					...
					<td class="breakTD">
						...
						<?php if ($sumBalance=='0') { ?>
							<?php
							$query = "SELECT COUNT(aus_assign_status) AS totalAssign,SUM(CASE WHEN aus_assign_status= '1' THEN 1 ELSE 0 END) AS assignedNo FROM austria_clients_programs".$_SESSION['dbNo']." WHERE close='1' AND status='1' AND aus_change_program_status='0' AND aus_clients_id='".$clientID."' AND aus_program_assign='".$wtID."' ";
						...
						<br>
						<?php
							$query="SELECT COUNT(aus_direct_applied_status) AS totalAssign, SUM(CASE WHEN aus_direct_applied_status='9' THEN 1 ELSE 0 END) AS assignedNo FROM austria_clients_programs".$_SESSION['dbNo']." WHERE close='1' AND status='1' AND aus_change_program_status='0' AND aus_clients_id='".$clientID."' AND aus_program_assign='".$wtID."' ";
							$result = mysqli_query($con, $query);
							$data = mysqli_fetch_array($result);
							$totalNo = $data['totalAssign'];
							$assignedNo = $data['assignedNo'];
						...
						<br>
						<?php
						$query="SELECT aus_direct_applied_status, aus_program_assign_date FROM austria_clients_programs".$_SESSION['dbNo']." WHERE close='1' AND status='1' AND aus_change_program_status='0' AND aus_clients_id='".$clientID."' AND aus_program_assign='".$wtID."' ";
						$result_ex = mysqli_query($con, $query);
