			...
			<table id="datatable" class="table table-bordered nowrap text-center" style="width: 100%; table-layout: fixed;">
				...
				foreach ($clientData_ex as $row) {
					...
					if ($row['client_country']=='italy') {
						$result = mysqli_query($con, "SELECT COUNT(italy_checklist_name) AS NumberOfLists FROM italy_clients_checklist".$_SESSION['dbNo']." WHERE close='1' AND status = '1' AND italy_checklist_name='Cimea' AND italy_client_check_id='".$clientID."' AND entry_by='".$_SESSION['user_id']."' ");
					...
					<tr id="<?php echo $row['client_id']; ?>">
						...
						<td>
						<?php
							$result = mysqli_query($con, "SELECT COUNT(italy_checklist_name) AS NumberOfLists FROM italy_clients_checklist".$_SESSION['dbNo']." WHERE close='1' AND status='1' AND italy_client_check_id='".$clientID."' AND (italy_checklist_name='Visa' || italy_checklist_name='Cimea') AND entry_by='".$_SESSION['user_id']."' ");
							$data = mysqli_fetch_assoc($result);
					...
					<?php	
					$sr++; }elseif ($numItaly=='0' && $checklistStatus=='0'){
					?>
					<tr id="<?php echo $row['client_id']; ?>">
						<?php 
						echo "<td>".$sr."</td><td>ID-".$row['client_id']."<br><b>".date("d-m-Y", strtotime($row['create_date']))."</b><br>";
						...
						<td>
						<?php
							$result = mysqli_query($con, "SELECT COUNT(italy_checklist_name) AS NumberOfLists FROM italy_clients_checklist".$_SESSION['dbNo']." WHERE close='1' AND status='1' AND italy_client_check_id='".$clientID."' AND (italy_checklist_name='Cimea') AND entry_by='".$_SESSION['user_id']."' ");
							$data = mysqli_fetch_assoc($result);
							$num = $data['NumberOfLists'];
							// $isItaly = $num == 2 && $row['client_country'] == 'italy';
						?>
