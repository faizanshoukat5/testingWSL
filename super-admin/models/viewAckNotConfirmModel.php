...
if (isset($_POST['checkcountryName'])) {
	...
	$whereCondition = "close='1' AND status='1' AND admin_status='1' AND case_status='0' AND change_status='0' AND ack_confirm_status='0' ";
		...
		<table id="datatable" class="table table-bordered nowrap text-center" style="width: 100%; table-layout: fixed;">
			...
			while ($row = mysqli_fetch_assoc($clientData_ex)) {
				...
				<tr id="<?php echo $row['client_id']; ?>">
					...
					<td>
					<!-- ack button -->
					<button type="button" class="btn <?= $row['ack_confirm_status']==1 ? 'btn-success' : 'btn-outline-danger' ?> btn-sm mt-1" data-toggle="tooltip" data-placement="top" title="Add Acknowledge From Client" onclick="ackConfirm(<?= $row['client_id'] ?>);"> <i class="mdi <?= $row['ack_confirm_status']==1 ? 'mdi-check-circle' : 'mdi-close-circle' ?>"></i> Ack. </button>
					<br>

					<?php if($row['client_pro_confirm_status'] == 1 && $row['ack_confirm_status'] ==1 && $row['ack_confirm_status'] == 1 && $row['client_document_status'] ==1) {?>
						<span class="badge badge-success" data-toggle="tooltip" data-placement="top" title="Forwarded to Admission Head">Forwarded</span>
					<?php }elseif($row['client_pro_confirm_status']==1 || $row['ack_confirm_status']==1 || $row['ack_confirm_status']==1 || $row['client_document_status']==1){?>
						<span class="badge badge-dark" data-toggle="tooltip" data-placement="top" title="Process">Process</span>
					<?php }if($row['client_pro_confirm_status']==0 && $row['ack_confirm_status']==0 && $row['ack_confirm_status']==0 && $row['client_document_status']==0) {?>
