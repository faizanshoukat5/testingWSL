	...
	<div class="row">
		<div class="col-md-12">
			<div class="row">
				...
				<?php 
				$clientData = "SELECT italy_change_program_name, italy_university_name, italy_program_name FROM italy_clients_programs".$_SESSION['dbNo']." WHERE close='1' AND status='1' AND italy_clients_id='".$clientID."' AND italy_program_assign!='0' ";
				$clientData_ex = mysqli_query($con, $clientData);
				foreach ($clientData_ex as $row) {
				...
				<div class="col-md-6">
					<div class="alert bg-dark text-warning">
						<p>Program: <strong><?php if($row['italy_change_program_name']!=''){ ?>
							<del><?php echo ucwords($row['italy_program_name']); ?></del> / 
							<?php echo ucwords($row['italy_change_program_name']); ?>
							<?php
							}else{
								echo ucwords($row['italy_program_name']);
							}
							?></strong> </p>
					</div>
				</div>
		...
		<div class="table-responsive">
			<table class="table table-bordered nowrap text-center" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
				...
				<tbody>
					...
					<?php } ?>
					<?php if ($clientCountryApply=='austria' || $clientCountryApply=='italy') { ?>
					<form action="" method="POST" enctype="multipart/form-data">
						<input type="hidden" name="updateID" value="<?php echo $clientID;?>" id="clientID">
						...
						<tr id="trDoc37" style="<?php echo $admissionDoc37!='' ? 'background: #52c234; font-weight: 600;' : 'font-weight: 600;'; ?>">
							<td><?php if($clientCountryApply=='austria'){echo "31";}else{echo "34";}?></td>
							<td>Curriculum </td>
							<td>
	...
	function uploadDocument(id) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				$("#trDoc"+id).css({ "background": "#52c234", "font-weight": "600" });
			}
		});
	}
